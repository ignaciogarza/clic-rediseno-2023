...
<head>
	...
	function search(page){
		...
		$.ajax({
	        ...
	        dataType : 'json', 
	        data : data,
	        url : '/community/communitySkillSendList',
	        success : function(response) {
	          ...
	          if(list != null && list.length > 0){	
	        	  ...
	        	  for (var i = 0; i < list.length; i++) {
	        		  ...
	        		  if(list[i].timeCheck  == 'N'){
	        		  	html += '				<p class="text"><spring:message code="community.message7" /></p>';
	        		  }else{
	        			html += '				<p class="text"><spring:message code="community.message8" /></p>';  
	        		  }
	        		  html += '				<div class="mt-2">';
	        		  if(list[i].timeCheck  == 'N'){
	        			  html += '				<button type="button" class="btn btn-sm btn-outline-gray btn-request" onclick="fnAgainSkillFriendAuth(\''+list[i].userId+'\', \''+list[i].skillCode+'\', \''+list[i].examClassCode+'\')">Recordar</button>';
	        		  } 
	        		  html += '					<button type="button" class="btn btn-sm btn-outline-gray btn-cancle2" onclick="fnDelSkillFriendAuth(\''+list[i].userId+'\', \''+list[i].skillCode+'\', \''+list[i].examClassCode+'\')"><spring:message code="button.cancel" text=""/></button>';
	        		  html += '				</div>';
	        		  html += '			</div>';
	...
	//요청 취소 
	function fnDelSkillFriendAuth(friendId, skillCode, examClassCode){
		var data = {
				userId : userId,
				friendId : friendId,
				skillCode : skillCode,
		...
		$.ajax({
			url: '/community/deleteSkillFriendAuth',
			type: 'post',
			data: data,
			dataType: 'json',
			success: function(response){
			...
			<article id="content">
				...
				<div class="content-fixed community-total">
					...
					<div class="content-body style2">
						...
						<div class="tab-content" id="skillSendForm">
