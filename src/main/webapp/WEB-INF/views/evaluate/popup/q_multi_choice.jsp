			...
			<div class="question-area">
				...
				<div class="item">
					...
					<form id="answerForm">
						...
						<input type="hidden" id="multiChoiceTest" value="multiChoiceTest">
						<ul class="answer-list multiChoice">
							<%-- c:forEach items="${data.exampleList1}" var="item">
								<li class="check-item">
									<input type="checkbox" name="exampleAnswer" id="input_${item.exampleNumber}" value="${item.exampleNumber}">
									<label for="input_${item.exampleNumber}">
										<strong>${item.exampleNumber}.</strong>${item.exampleContents}
										<c:if test="${not empty item.exampleImagePath}">
											<span class="answer-image">
												<img src="${item.exampleImagePath}" alt="" onload>
											</span>
										</c:if>
									</label>
								</li>
