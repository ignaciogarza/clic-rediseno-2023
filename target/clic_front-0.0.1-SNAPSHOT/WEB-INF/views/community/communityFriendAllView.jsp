...
<head>
	...
	function search(page){
		...
		$.ajax({
	        ...
	        dataType : 'json', 
	        data : data,
	        url : '/community/communityFriendAllList',
	        success : function(response) {
	          ...
	          if(list != null && list.length > 0){	
	        	  ...
	        	  for (var i = 0; i < list.length; i++) {						
	        		  ...
	        		  html += '				<button type="button" class="btn btn-delete" onclick="fuFriendDel(\''+list[i].friendId+'\')">delete</button>';
	        		  html += '			</div>';
	        		  html += '		</div>';
	        		  html += '	</div>';
				  }
	...
	//포토폴리오  '${list.userImagePath}', '${list.countryCode}'
	...
	function fnPortfolioList111111(friendId, name, firstName, jobNameEng, friendStatusCode){
		$("#fullName").text(name+" "+firstName);
		$("#popJobName").text(jobNameEng);
