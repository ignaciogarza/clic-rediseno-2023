...
<head>
	...
	<script>
		...
		var questionId = getParameterByNames('questionId');
		...
		$(document).ready(function(){
			...
			firstSurveyExampleSetting(questionId);
		...
		//설문조사 문항 최초설정
		function firstSurveyExampleSetting(questionId){
		    ...
		    $.ajax({
		        ...
		        dataType : 'json', 
		        data : data,
		        url : '/dashboard/dashboardSurveyDetailViewList',
		        success : function(response) {
					...
					for (var i = 0; i < resultList.length; i++) {
						var exampleList = resultList[i].exampleList;
						selectHtml += '<li class="on" value="'+resultList[i].questionId+'" onclick="searchSurveyList(this);" id="questionId_'+resultList[i].questionId+'"><a href="#;" onclick="textSurveyValue(this);">'+resultList[i].questionTitleEng+'</a></li>';
						for (var j = 0; j < exampleList.length; j++) {
							selectHtml += '<input type="hidden" id="emamples_'+exampleList[j].questionId+'_'+j+'" value="'+exampleList[j].exampleId+'">';
						}				
					}
					
					$("#selectForm").append(selectHtml);
					
					var questionText = $("#questionId_"+questionId+" a").text();
					$("#questionText").text(questionText);
					
					
					var html = '';
					$("#exampleListView").empty();			
					for (var i = 0; i < resultList.length; i++) {	    		
						if(resultList[i].questionId == questionId){
							...
							for (var j = 0; j < exampleList.length; j++) {  
								...
								html += '<li name="emampleOn" id="emample_'+j+'" data-exampleid="'+exampleList[j].exampleId+'">';
								html += '	<a href="#;" onClick="fnTabMenuClick('+j+')">';
								html += '		<div class="info-area">';
								html += '			<span class="title">'+exampleList[j].exampleTitleEng+'</span>';
								html += '			<div class="figure">';
								html += '				<span class="number">'+exampleList[j].answerCnt+'</span>';
					...
					$("#exampleListView").append(html);
					
					$("#emample_0").addClass("on");
					 
					 //초기 exampleId 값 세팅 
					var exampleId = $("#emample_0").data("exampleid");			 
					$("#exampleIds").val(exampleId);
					getList();
		       },error:function(){
		...
		//설문문항 제목
		function textSurveyValue(value) {
			var code = $(value).text();
			$("#questionText").text(code);
		}

		//설문 문항 검색
		function searchSurveyList(value) {
			var questionId = $(value).val();			
			$("#questionId").val(questionId);
			
			var emamplesId = $("#emamples_"+questionId+"_0").val();
			$("#exampleIds").val(emamplesId);		
			console.log(searchType);
			//page = 1; //1로 초기화
			//getList(page, skillCode, examClassCode);
			//초기화
			$("#searchType").val("0");
			$("#searchText").text('<spring:message code="dashboard.text22" text="" />');
		...
		//var exampleIds;
		//상단 탭 클릭 
		function fnTabMenuClick(index){
			$("li[name='emampleOn']").removeClass("on");
			//$("#emampleOn").removeClass("on");
			 $("#emample_"+index).addClass("on");
			 
			var exampleIds =  $("#emample_"+index).data("exampleid");
			//alert(exampleId);
			$("#exampleIds").val(exampleIds);			
		...
		function getList(cityText){
		    ...
		    $.ajax({
		        ...
		        dataType : 'json', 
		        data : data,
		        url : '/dashboard/dashboardSurveyDetailList',
		        success : function(response) {
			        ...
			        surveyExampleSetting(result.resultList);
		...
		function surveyExampleSetting(resultList){
			
			var questionId = $("#questionId").val();
			var exampleIds = $("#exampleIds").val();
			...
			for (var i = 0; i < resultList.length; i++) {	    		
				if(resultList[i].questionId == questionId){
					...
					var exampleList = resultList[i].exampleList;
					for (var j = 0; j < exampleList.length; j++) {	  
						if(exampleList[j].exampleId == exampleIds){
							html += '<li class="on" name="emampleOn" id="emample_'+j+'" data-exampleid="'+exampleList[j].exampleId+'">';
						}else{
							html += '<li name="emampleOn" id="emample_'+j+'" data-exampleid="'+exampleList[j].exampleId+'">';
						}
						//html += '<li name="emampleOn" id="emample_'+j+'" data-exampleid="'+exampleList[j].exampleId+'">';
	...
	<div class="wrapper gvm-wrapper gvm-login"><!-- // 정부회원 로그인시 클래스 .gvm-login 추가 (로그아웃 또는 일반회원 로그인시 클래스 삭제) -->
		...
		<div id="container">
			<article id="content">
				...
				<div class="gv-area survey-detail">
					<div class="gv-ctrl d-up-lg">
						<div class="gv-ctrl-inner">
							<div class="left">
								<dl class="dropdown dropdown-gv">
									<input type="hidden" id="questionId" value="">
									<dt><a href="#;" class="btn" id="questionText">Ya sea para tomar ciencia de la información</a></dt>
									<dd>
							...
							<div class="right">
								<a href="/dashboard/dashboardSurveyView" class="btn btn-list">Lista</a>
							</div>
						</div>
					</div>
					...
					<nav class="tab-menu">
					<input type="hidden" id="exampleIds" value="">
						<ul class="tab-list" id="exampleListView">							
