...
<%-- <c:set var="len" value="${fn:length(data.exampleList1)}"/> --%>
						<div class="line-answer mt-lg-3 mt-md-4">
							<input type="hidden" id="lineDrawingTest" value="lineDrawingTest">
							<%-- <c:if test="${len > 0}">
								<c:forEach begin="0" end="${len - 1}" var="i">
									<c:set var="right" value="${data.exampleList1[i]}"/>
									<c:set var="left" value="${data.exampleList2[i]}"/>
									<div class="nodes">
						
										<div class="group1">
											<label class="node">
												<span class="node-inner">
													${left.exampleContents}
													<c:if test="${not empty left.exampleImagePath}">
														<span class="answer-image">
															<img src="${left.exampleImagePath}" alt="" onload="drawing('left_${left.exampleNumber}')">
														</span>
													</c:if>
													<span class="check-item">
														<input type="radio" data-connect="false" id="left_${left.exampleNumber}" data-id="left_${left.exampleNumber}" data-use="false" value="${left.exampleNumber}">
										...
										<div class="group2">
											<label class="node">
												<span class="node-inner">
													${right.exampleContents}
													<c:if test="${not empty right.exampleImagePath}">
														<span class="answer-image">
															<img src="${right.exampleImagePath}" alt="" onload="drawing('right_${right.exampleNumber}')">
														</span>
													</c:if>
													<span class="check-item">
														<input type="radio" data-connect="false" id="right_${right.exampleNumber}" data-id="right_${right.exampleNumber}" data-use="false" name="exampleAnswer${right.exampleNumber}">
														<span class="radio-custom"></span>
													</span>
												</span>
											</label>
							...
							<div class="btn-group-default mt-lg-8 mt-md-4 text-right line-answer">
								<button type="button" class="btn btn-md btn-outline-gray btn-round btn-canvas-reset"><spring:message code="evaluate.question.draw-cancel" text="연결 취소" /></button>
							</div>
						</div>
					</form>
				</div>
